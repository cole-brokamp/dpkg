...
print.dpkg <- function(x, ...) {
  if (interactive()) cli::cli_text("# [{cli::symbol$menu}] {attr(x, 'name')}-v{attr(x, 'version')}")
  if (length(attr(x, "title") > 0)) {
    cli::cli_text("# title: \"{attr(x, 'title')}\" ")
  }
  if (length(attr(x, "homepage") > 0)) {
    cli::cli_text("# homepage: {attr(x, 'homepage')} ")
  }
  if (interactive()) cli::cli_text("# {cli::symbol$info} Use `dpkg_meta() to get all metadata")
